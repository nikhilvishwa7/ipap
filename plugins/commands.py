...
@Client.on_message(filters.command("start") & filters.incoming)
    if FSUB and not await is_subscribed(client, message):
        try:
            invite_link = await client.create_chat_invite_link(int(FSUB))
        ...
        await client.send_message(
            ...
            text="**𝙁𝙞𝙧𝙨𝙩, 𝙅𝙤𝙞𝙣 𝙊𝙪𝙧 𝙐𝙥𝙙𝙖𝙩𝙚 𝘾𝙝𝙖𝙣𝙣𝙚𝙡 𝙩𝙝𝙚𝙣 𝘾𝙡𝙞𝙘𝙠 𝙤𝙣 𝙏𝙧𝙮 𝙖𝙜𝙖𝙞𝙣 𝙛𝙤𝙧 GET 𝙮𝙤𝙪R 𝙙𝙚𝙨𝙞𝙧𝙚𝙙 𝙢𝙤𝙫𝙞𝙚/𝙬𝙚𝙗𝙨𝙚𝙧𝙞𝙚𝙨 ♥️**",
            reply_markup=InlineKeyboardMarkup(btn),
            parse_mode=enums.ParseMode.MARKDOWN
            )
        return
async def start(client, message):
        return    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [
    ...
    if data.split("-", 1)[0] == "BATCH":
        sts = await message.reply("<b>Asessing Files.../</b>")
